...
class AppSharedPreferences {
    ...
    companion object {
        ...
        fun setUserPermissoes(context: Context, permissoes: Set<String>){

            val prefs: SharedPreferences = context.getSharedPreferences(SESSION, Context.MODE_PRIVATE)

            val editor: SharedPreferences.Editor = prefs.edit()
            editor.putStringSet(KEY_USER_PERMISSIONS, permissoes)
            editor.apply()
        }

        fun getUserPermissoes(context: Context): Set<String>{

            val prefs: SharedPreferences = context.getSharedPreferences(SESSION, Context.MODE_PRIVATE)
            val conjunto = prefs.getStringSet(KEY_USER_PERMISSIONS, setOf())
            return conjunto ?: setOf()
        }
    }
